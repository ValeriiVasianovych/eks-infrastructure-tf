resource "aws_acm_certificate" "grafana_domain_cert" {
  domain_name       = "grafana.${var.env}.${data.aws_route53_zone.main.name}"
  validation_method = "DNS"

  tags = {
    Name = "grafana-prod-certificate"
  }
}

resource "aws_route53_record" "grafana_domain_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.grafana_domain_cert.domain_validation_options :
    dvo.domain_name => {
...
resource "aws_route53_record" "grafana_domain_record" {
  ...
  zone_id = data.aws_route53_zone.main.id
  name    = "grafana.${var.env}.${var.hosted_zone_name}"
  type    = "A"
